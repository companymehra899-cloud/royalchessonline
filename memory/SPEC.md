...
- Auth: email/password, guest, and direct Google OAuth authorization-code exchange. Google requires `GOOGLE_CLIENT_ID` and `GOOGLE_CLIENT_SECRET`; the redirect URI is the web app origin.
...
- Web OAuth callback resolves to the app origin root (no path), e.g. `https://<preview-host>/`.